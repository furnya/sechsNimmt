...
export class BoardComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.gameStateSub = this.gameService.gameStateChanged.subscribe(
      (gameState) => {
        if (
          this.gameService.playerIndex !== NO_PLAYER_INDEX &&
          gameState?.finished
        ) {
          this.gameIsFinished(gameState);
        } else {
        ...
        if (
          ...
          this.choosingAndNoCardSelected();
        } else {
          ...
          this.setTurnHint(gameState);
        }
      }
    );
  }

  setTurnHint(gameState: GameState) {
  ...
  startTimeout() {
    ...
    this.timeout = setTimeout(() => {
      ...
      const cardIndex = Math.floor(
        Math.random() * this.gameService.getHandCards().length
      );
      this.gameService
        .selectCard(this.gameService.getHandCards()[cardIndex])
        .then(() => {
          this.shouldSelectRandomCard = true;
        });
  ...
  choosingAndNoCardSelected() {
    this.hintOpen = true;
    this.hint = CHOOSE_HINT;
    if (this.gameService.selectedCardLocally !== NO_CARD_SELECTED) {
      this.gameService.selectCard(this.gameService.selectedCardLocally);
    } else if (
      !this.timeout &&
      !this.gameService.options.thinkingTimeoutDisabled.value
    ) {
      this.startTimeout();
    }
  }

  gameIsFinished(gameState: GameState) {
    if (this.dialogOpen) {
      this.dialogRef.componentInstance.data = gameState;
    } else {
      this.dialogOpen = true;
      this.dialogRef = this.finishedGameDialog.open(FinishedDialogComponent, {
        data: gameState,
        disableClose: true,
